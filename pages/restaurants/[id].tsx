...
export default function Home() {
  ...
  useEffect(() => {
    if (restaurant) {
      const greeting = translate("greeting", language)[0].replace(
        "{restaurant_name}",
        restaurant.name
      );
  ...
  useEffect(() => {
    if (messageState.pending === undefined) {
      setImages([
    ...
    setShowImages(true);
  }, [showImages, messageState.pending]);
  ...
  const handleSubmit = async (e: any, restaurantData: Restaurant | null) => {
    ...
    fetchEventSource("/api/chat", {
      ...
      onmessage: (event) => {
        ...
        setShowImages(false);
        if (event.data === "[DONE]") {
          console.log("Before updating state:", messages, pending);
          setMessageState((state) => ({
          ...
          console.log("pending:", messageState.messages);
        }
      },
    });
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Umami AI is the assistant every restaurant deserves."
        />
        ...
        <meta
          property="og:title"
          content="UmamiAI: AI assistant every restaurant deserves."
        />
        <meta
        ...
        <div className={styles.cloud}>
          <div ref={messageListRef} className={styles.messagelist}>
            {chatMessages.map((message, index) => {
              ...
              if (message.type === "apiMessage" && index !== 0) {
                return (
                  <div key={index} className={className}>
                    <div key={index} className={styles.iconwithmessage}>
                      {icon}
              ...
              <textarea
                ...
                placeholder={
                  loading
                    ...
                    : `${translate("placeholder", language)[0]}`
                }
                value={userInput}
